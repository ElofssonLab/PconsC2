...
if not os.path.exists(seqfile + '.jhE0.jones'):
        t = check_output([jackhmmer, '--cpu', '4', '-N', '5', '--incE', '1', '-E', '1', '-A', seqfile +'.jhE0.ali', seqfile + '.fa', uniref100])
        check_output([reformat, 'sto', 'fas', seqfile + '.jhE0.ali', seqfile + '.jhE0.fas'])
        check_output(['rm', seqfile + '.jhE0.ali'])

if not os.path.exists(seqfile + '.jhE0.psicov'):
        t = check_output([trim, seqfile + '.jhE0.fas'])
	f = open(seqfile + '.jhE0.jones', 'w')
	f.write(t)
	f.close()
	print "Running psicov"
	if not os.path.exists(seqfile + '.jhE0.psicov'):
		t = check_output([psicov, '-j', '1', seqfile + '.jhE0.jones'])
		f = open(seqfile + '.jhE0.psicov', 'w')
		f.write(t)
		f.close()

t = check_output([trim2, seqfile + '.jhE0.fas'])
f = open(seqfile + '.jhE0.trimmed', 'w')
f.write(t)
f.close()

os.chdir(os.path.abspath(infile)[:os.path.abspath(infile).rfind('/')])
infilestem = infile.split('/')[-1] + '.jhE0'
