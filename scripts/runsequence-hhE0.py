...
print 'HHblits'
if not os.path.exists(seqfile + '.hhE0.jones'):
	t = check_output([hhblits, '-n', '5', '-all', '-maxfilt', '999999', '-realign_max', '999999', '-oa3m', seqfile +'.hhE0.a3m', '-e', '1', '-i', seqfile + '.fa', '-d', '/triton/ics/work/mjs/uniprot20_2013_03/uniprot20_2013_03'])
	check_output([reformat, 'a3m', 'fas', seqfile + '.hhE0.a3m', seqfile + '.hhE0.fas'])

if not os.path.exists(seqfile + '.hhE0.psicov'):
        t = check_output([trim, seqfile + '.hhE0.fas'])
	f = open(seqfile + '.hhE0.jones', 'w')
	f.write(t)
	f.close()
	print "Running psicov"
	if not os.path.exists(seqfile + '.hhE0.psicov'):
		t = check_output([psicov, '-j', '1', seqfile + '.hhE0.jones'])
		f = open(seqfile + '.hhE0.psicov', 'w')
		f.write(t)
		f.close()

t = check_output([trim2, seqfile + '.hhE0.fas'])
f = open(seqfile + '.hhE0.trimmed', 'w')
f.write(t)
f.close()

os.chdir(os.path.abspath(infile)[:os.path.abspath(infile).rfind('/')]) 
infilestem = infile.split('/')[-1] + '.hhE0'
