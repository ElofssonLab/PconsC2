...
print 'HHblits'
if not os.path.exists(seqfile + '.hhE4.jones'):
	t = check_output([hhblits, '-n', '5', '-all', '-maxfilt', '999999', '-realign_max', '999999', '-oa3m', seqfile +'.hhE4.a3m', '-e', '1e-4', '-i', seqfile + '.fa', '-d', '/triton/ics/work/mjs/uniprot20_2013_03/uniprot20_2013_03'])
	check_output([reformat, 'a3m', 'fas', seqfile + '.hhE4.a3m', seqfile + '.hhE4.fas'])

if not os.path.exists(seqfile + '.hhE4.psicov'):
        t = check_output([trim, seqfile + '.hhE4.fas'])
	f = open(seqfile + '.hhE4.jones', 'w')
	f.write(t)
	f.close()
	print "Running psicov"
	if not os.path.exists(seqfile + '.hhE4.psicov'):
		t = check_output([psicov, '-j', '1', seqfile + '.hhE4.jones'])
		f = open(seqfile + '.hhE4.psicov', 'w')
		f.write(t)
		f.close()

t = check_output([trim2, seqfile + '.hhE4.fas'])
f = open(seqfile + '.hhE4.trimmed', 'w')
f.write(t)
f.close()

os.chdir(os.path.abspath(infile)[:os.path.abspath(infile).rfind('/')])
infilestem = infile.split('/')[-1] + '.hhE4'
