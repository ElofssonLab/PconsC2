...
if not os.path.exists(seqfile + '.jhE10.jones'):
        t = check_output([jackhmmer, '--cpu', '4', '-N', '5', '--incE', '1e-10', '-E', '1e-10', '-A', seqfile +'.jhE10.ali', seqfile + '.fa', uniref100])
        check_output([reformat, 'sto', 'fas', seqfile + '.jhE10.ali', seqfile + '.jhE10.fas'])
        check_output(['rm', seqfile + '.jhE10.ali'])

if not os.path.exists(seqfile + '.jhE10.psicov'):
        t = check_output([trim, seqfile + '.jhE10.fas'])
	f = open(seqfile + '.jhE10.jones', 'w')
	f.write(t)
	f.close()
	print "Running psicov"
	if not os.path.exists(seqfile + '.jhE10.psicov'):
		t = check_output([psicov, '-j', '1', seqfile + '.jhE10.jones'])
		f = open(seqfile + '.jhE10.psicov', 'w')
		f.write(t)
		f.close()

t = check_output([trim2, seqfile + '.jhE10.fas'])
f = open(seqfile + '.jhE10.trimmed', 'w')
f.write(t)
f.close()

os.chdir(os.path.abspath(infile)[:os.path.abspath(infile).rfind('/')])
infilestem = infile.split('/')[-1] + '.jhE10'
